...
def _make_test_dokument(typ: Doktyp = Doktyp.ENTWURF) -> Dokument:
    return Dokument(
        titel="Gesetz zur Weiterentwicklung des Klimaschutzes",
        volltext="",
        hash="",
        typ=typ,
        ...
        link=SAMPLE_PDF_URL,
        autoren=[Autor(organisation="Fraktion GRÜNE")],
        drucksnr="17/10266",
    )
...
class TestEntwurfEnrichment:
    ...
    async def test_parlis_metadata_preserved(self):
        ...
        assert enriched.titel == "Gesetz zur Weiterentwicklung des Klimaschutzes"
