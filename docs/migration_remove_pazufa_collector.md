...
- **3.1 `pyproject.toml`** — removed the `collector` and `openapi-client` path deps. `pazufa-corelib` switched from `{path = "../pazufa-scraper-core", develop = true}` to `{git = "https://codeberg.org/PaZuFa/pazufa-scraper-core.git", rev = "v0.1.2"}` (satisfies C6/D5; path install is now a commented dev-group override). Added the deps that `collector` used to provide transitively and are now imported directly: `httpx` (`bawue.api`), `redis` (`bawue.cache`), `python-dotenv` (`bawue.__main__`), `kreuzberg` (`bawue_dok`). `redis` pinned `>=7.4` (doc's `^7.4` was stale — the resolved wheel is 8.x). `poetry.lock` regenerated; resolved corelib to commit `aac0953`. Distribution `name` left as `pazufa-bawue-scraper` (a rename has no functional benefit and would churn artifact paths — deviates from the doc's "bump to bawue-scraper" suggestion); `description` updated to drop "collector framework".
- **3.2 `Makefile`** — `install` no longer rsyncs `../pazufa-collector`/`../pazufa-scraper-core` into `vendor/` nor runs `npx openapi-generator-cli` (corelib is fetched from its git tag by `poetry install`). `run` → `python -m bawue`. `package`/`clean` comments and the `vendor` cleanup entry updated.
- **3.3 `Dockerfile` / `docker-compose.yml`** — dropped both `COPY vendor/pazufa-collector` / `vendor/pazufa-scraper-core` steps (builder and runtime stages) since corelib is now a regular git-installed package (not editable). Added `git` to the builder apt list so `poetry install` can fetch the git dep. ENTRYPOINT/entrypoint `python -m collector` → `python -m bawue`.
- **3.5 `README.md`** — reframed from "collector plugin / framework-managed" to "self-contained scraper depending only on pazufa-scraper-core"; dropped the "clone pazufa-collector alongside" setup and the vendor language; `python -m collector` → `python -m bawue` throughout; Links entry swapped collector → scraper-core. (Backend-domain "collector" mentions — the API-key `collector` scope, `COLLECTOR_ID`, `collector-uuid` — are unrelated and kept.)
- **3.6 `docs/architecture.md`** — §1 rewritten as self-contained; the "pazufa-collector Framework" subgraph became "BaWue Runtime (bawue.*)"; §2 dependency table drops `openapi-client`/`collector-core` rows and adds `pazufa-scraper-core`; §3 title + `ScraperCache`→`BawueCache`; the "Framework-Provided Capabilities" table and error-handling table now attribute behaviour to the concrete `bawue.*` modules.
- **Pulled forward from Phase 4 to keep the tree green** (unavoidable — removing the deps from `pyproject.toml` breaks unit-test *collection*, since pytest imports every `conftest.py` regardless of markers): **4.2 slice** — `tests/integration/conftest.py` no longer imports `collector`/`openapi_client` at module scope; the three imports moved inside the `collector_config` fixture (the fixture body is otherwise unchanged; the full rewrite onto `bawue.config`/`bawue.cache` is still Phase 4.2). **4.3 slice** — `.woodpecker.yml` lost the `generate-openapi-client` step and both collector/corelib `git clone` lines (corelib arrives via `poetry install`), and the `vendor-prep` step (the Dockerfile no longer reads `vendor/`); the three `docker-*` steps now `depends_on: [pytest, ruff-lint, ruff-format]`. `cloudbuild.yaml` needed no change (it only `docker build`s).
- **Not touched (still Phase 4):** `vendor/pazufa-collector/` still on disk; `tests/integration/conftest.py` still *runs* against the old packages (deselected by default); C3 cutover-parity harness; `bawue.types` alias decision.
- Verified: `poetry lock` + `poetry sync` (removes `collector`/`openapi_client` from the venv), then `import collector` fails while `import pazufa_corelib` / `import bawue` succeed; `python -m bawue --help` and `--dump-config` against `config.dev.toml`; **822 unit tests pass with `collector` absent**; ruff lint + format clean.
