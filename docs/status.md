...
| Canonical name normalization | ❌ Missing         | DoD SHOULD requirement — no consolidation mechanism exists for `Autor.organisation` variants (Roadmap #10 G2). Production Fraktion strings are PARLIS-consistent today but not enforced.                                            |
| Reserved entity names        | ⚠️ Partial         | `ReservedGremium` StrEnum (DD-021) used at 3 sites: vorgaenge plenarprotokoll, ICS Plenarsitzung, Beteiligung. **Open:** `Landtag` default (G1.4 — backend dedup risk), wiki/spec discrepancy on `gesetzesblatt` vs. `volk` (G4). |
...
| 10 | DoD: Verify reserved names + canonical normalization | Low               | **Partial** (DD-021): `ReservedGremium.PLENUM`/`REGIERUNG` now used for plenarprotokoll, ICS Plenarsitzung, and Beteiligung-Station. Open: G1.4 `Landtag` default → `plenum` (~1 000 stations, backend dedup confirmation needed); G2 canonical normalization for `Autor.organisation` (no mechanism); G3 `Ausschusssitzungen` violates name-specificity rule (`ics_parser.py:49`); G4 wiki/spec discrepancy on `gesetzesblatt` vs. `volk`.                                                                              |
